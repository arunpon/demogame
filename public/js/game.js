$(document).ready(function() {
  ...
  function renderChildren() {
    $containers.each(function(container_i) {
      ...
      for (var i = 0; i < 12; i++) {
        ...
        $(this).append(element);
        if (!randomArray[i]) {
          var shapeRand = getRandom(0, 2),
            colorRand = getRandom(0, 2),
            fillRand = getRandom(0, 2),
            countRand = getRandom(0, 2);
  ...
  function drawTriangle(element, color, fillStyle, shapeNum) {
    ...
    for (var i = 1; i <= shapeNum + 1; i++) {
      var lineObj = {
        strokeWidth: 4,
        x1: (xPos - (shapeWidth / 2)),
        y1: yPos,
        x2: xPos,
        y2: (yPos - (shapeHeight / 2)),
        x3: (xPos + (shapeWidth / 2)),
        y3: yPos,
        x4: xPos,
        y2: (yPos + (shapeHeight / 2)),
      };
